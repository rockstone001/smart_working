$(function () {
    ...
    $('#btn_edit').click(function() {
        var toRemoveElems = $('#post_list').bootstrapTable('getSelections');
        if (toRemoveElems.length == 0) {
            $('#operate_dialog .modal-body').html('请选择要编辑的订单!');
            $('#operate_dialog').modal({backdrop: 'static'});
            setTimeout(function(){$("#operate_dialog").modal("hide")}, 2000);
        } else if (toRemoveElems.length > 1) {
            $('#operate_dialog .modal-body').html('一次只能编辑一个订单!');
            $('#operate_dialog').modal({backdrop: 'static'});
            setTimeout(function(){$("#operate_dialog").modal("hide")}, 2000);
        } else {
    ...
    $('#btn_detail').click(function() {
        var toRemoveElems = $('#post_list').bootstrapTable('getSelections');
        if (toRemoveElems.length == 0) {
            $('#operate_dialog .modal-body').html('请选择要查看的订单!');
            $('#operate_dialog').modal({backdrop: 'static'});
            setTimeout(function(){$("#operate_dialog").modal("hide")}, 2000);
        } else if (toRemoveElems.length > 1) {
            $('#operate_dialog .modal-body').html('一次只能查看一个订单!');
            $('#operate_dialog').modal({backdrop: 'static'});
            setTimeout(function(){$("#operate_dialog").modal("hide")}, 2000);
        } else {
            location.href = $(this).attr('data-link') + '/' + toRemoveElems[0].id;
        ...
        $('#post_list').bootstrapTable({
            ...
            detailFormatter:function(index, row){
                ...
                $.each(row, function (key, value) {
                    if (key == 0) return;
                    html.push('<p><span style="display: inline-block; width:120px; font-weight: 600;">' + columns[key] + ':</span> ' + value + '</p>');
                });
                return html;
            },
            columns: [{
